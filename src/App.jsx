...
function App() {
    console.log('App.jsx: Component initialized');
    ...
    useEffect(() => {
        console.log('App.jsx: useEffect mount');
        fetchData();
    }, []);
    ...
    const findVal = (row, possibleKeys) => {
        ...
        for (const key of possibleKeys) {
            ...
            // Exact match (already trimmed in normalization step, but good safety)
            if (row[key] !== undefined) return row[key];

            // Case insensitive match
            const foundKey = rowKeys.find(k => k.toLowerCase() === cleanKey);
    ...
    const getJsDate = (row) => {
        const val = findVal(row, ['F.Carga', 'Fecha Carga', 'Fecha', 'Date', 'Fech.Carga Ped.']);
    ...
    const { drivers, months, weeks } = useMemo(() => {
        ...
        data.forEach(row => {
            const conductor = findVal(row, ['Conductor', 'Chofer', 'Driver', 'Conductor 1', 'Nombre Conductor 1']);
            if (conductor) d.add(conductor);

            const date = getJsDate(row);
            if (date && !isNaN(date)) {
                const monthStr = getMonthIdentifier(date);
                m.add(monthStr);

                const weekStr = getWeekIdentifier(date);
                w.add(weekStr);
            }
        });
                ...
                <header className="mb-8 flex flex-col md:flex-row justify-between items-center gap-4 print:hidden">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            {loading ? (
                                <p className="text-sm text-blue-500 animate-pulse">Sincronizando...</p>
                            ) : errorMsg ? (
                                    ...
                                    <tbody className="divide-y divide-gray-100">
                                        {filteredData.map((row, idx) => (
                                            <tr key={idx} className="hover:bg-gray-50">
                                                ...
                                                {!selectedDriver && <td className="p-3 text-gray-700 font-medium">{findVal(row, ['Conductor', 'Chofer', 'Driver', 'Conductor 1', 'Nombre Conductor 1'])}</td>}
                                                <td className="p-3 text-gray-700">
                                                    <div className="font-medium">{findVal(row, ['Pobl. Carga', 'Población Origen', 'Origen', 'Emp.Origen'])}</div>
                                                    <div className="text-xs text-gray-500">{findVal(row, ['Ori.Emp', 'Empresa Origen', 'Emp.Origen'])}</div>
                                                </td>
                                                <td className="p-3 text-gray-700">
                                                    <div className="font-medium">{findVal(row, ['Pobl. Descarga', 'Población Destino', 'Destino', 'Pobl. 1ª Desc.'])}</div>
                                                    <div className="text-xs text-gray-500">{findVal(row, ['Des.Emp', 'Empresa Destino', 'Emp. Destino'])}</div>
                                                </td>
